...
import { beginRunPreparation, getLatestPreparation } from "@/lib/g4/app/run-access";
import type { TestSelection } from "@/lib/g4/preparation";
import { runErrorResponse } from "@/lib/g4/app/http";

/** GET /api/runs/:id/preparation — latest preparation generation status (polling). */
export async function GET(
  ...
  if (!authz.session) return NextResponse.json({ preparation: null });
  try {
    const preparation = await getLatestPreparation({ userId: authz.session.userId, auditFirmId: authz.session.auditFirmId }, params.id);
